...
function updateSvelteFiles() {
	[
		...
		{
			view: 'routes/subjects/index',
			vars: [{ name: 'posts', type: '{ subjects: string; title: string, html: any }[]' }],
			contextModule: [
				{
					js: '.then(r => r.json())',
				...
				{
					js: '.then(posts => {',
					ts: '.then((posts: { subjects: string; title: string, html: any }[]) => {'
				}
			]
		},
		{
			view: 'routes/subjects/[subjects]',
			vars: [{ name: 'post', type: '{ subjects: string; title: string, html: any }' }]
		}
